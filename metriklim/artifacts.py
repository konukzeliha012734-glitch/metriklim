...
def build_timeseries_png(data: pd.DataFrame) -> bytes:
    ...
    ax.set_title("Metriklim – İklim Zaman Serisi", color="#063447", fontsize=14, weight="bold")
    ax.set_xlabel("Tarih")
    ax.grid(alpha=0.20)
...
def geodata_to_gpkg(gdf: gpd.GeoDataFrame, point: tuple[float, float]) -> bytes:
    with tempfile.TemporaryDirectory(prefix="metriklim_gpkg_") as temp:
        path = Path(temp) / "metriklim-cbs.gpkg"
